    ...
    private fun initializeDeviceProtectedStorage(context: Context) {
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                ...
                val prefSets = listOf("pastiera_prefs", "app_list_cache_prefs", "recent_emojis_prefs")
                for (prefSet in prefSets) {
                    val prefs = deviceContext.getSharedPreferences(prefSet, Context.MODE_PRIVATE)
                    Log.i(TAG, "Device-protected storage initialized for ${prefSet} with ${prefs.all.size} settings" )
                }
    ...
    private fun migrateToDeviceProtectedStorage(context: Context) {
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                ...
                val prefSets = listOf("pastiera_prefs", "app_list_cache_prefs", "recent_emojis_prefs")
                for (prefSet in prefSets) {
                    val credentialPrefs = credentialContext.getSharedPreferences(prefSet, Context.MODE_PRIVATE)
                    val devicePrefs = deviceContext.getSharedPreferences(prefSet, Context.MODE_PRIVATE)
                    ...
                    if (devicePrefs.all.isEmpty() && credentialPrefs.all.isNotEmpty()) {
                        Log.i(TAG, "Migrating ${credentialPrefs.all.size} settings of ${prefSet} to device-protected storage")
                    ...
                    Log.i(TAG, "Settings migration completed for ${prefSet}")
                }
            }
        } catch (e: Exception) {
